...
--- Get the version of HayaOS
function shell.hayaver()
    return os.version()
end
...
--else
do --// HAD to do this or the fucking prompt will not show. So... yeah... use the API instead.
    local function show_prompt()
        term.setBackgroundColor(bgColour)
        term.setTextColour(promptColour)
        write("HayaOS@"..(os.getComputerLabel() and os.getComputerLabel():gsub(" ","-"):lower() or "localhost") .."$ ")
    ...
    term.setBackgroundColor(bgColour)
    term.setTextColour(promptColour)
    print("HayaOS Kernel Shell")
	print("Build version: "..os.version():gsub("HayaOS v", ""))
    term.setTextColour(textColour)
    ...
    while not bExit do
		...
		local result = read(nil, tCommandHistory, complete)
