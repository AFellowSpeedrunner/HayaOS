...
local function redrawMenu()
	...
	term.setBackgroundColor(bMenu and colors.purple or(status_ok and colors.purple or colors.red))
	term.setTextColor(colors.white)
    term.clearLine()

    -- Draw line numbers
    term.setCursorPos(w - #("textColour " .. y) + 2, h)
    term.setTextColour(textColour)
    term.write("LN ")
    term.setTextColour(textColour)
    term.write(y)
    ...
    if bMenu then
        ...
        for nItem, sItem in pairs(tMenuItems) do
            if nItem == nMenuItem then
				term.setBackgroundColor(colors.magenta) --// A slighly ligher version of purple
            else
